...
CREATE TABLE project_sos.service_events (
    ...
    -- Chat Server 전용
    channel LowCardinality(String) DEFAULT '',    -- LOBBY / TEAM / ALL / WHISPER
...
[Queue Server] → Docker stdout                 → Vector docker_logs source
...
# Queue Server 전용 필드 추출
if .service == "queue" {
  .queue_depth = to_int(parse_regex(.message, r'depth=(?P<v>\d+)').v) ?? 0
  .match_wait_ms = to_int(parse_regex(.message, r'wait_ms=(?P<v>\d+)').v) ?? 0
}

# Chat Server 전용 필드 추출
if .service == "chat" {
  .channel = parse_regex(.message, r'channel=(?P<v>\w+)').v ?? ""
  .rate_limit_hit = to_int(parse_regex(.message, r'rate_limit=(?P<v>\d+)').v) ?? 0
}
'''
...
Queue Server와 Chat Server는 Unity와 동일한 형식으로 stdout에 출력한다.
...
2026-03-16 10:00:05.200 | INFO    | [Queue] Status, depth=3, waiting=2
...
```
2026-03-16 10:01:01.100 | INFO    | [Chat] Message sent, channel=TEAM, session=abc-123
...
| Queue | Match, Queue, Auth, System | 매칭, 큐 상태, 인증, 시스템 |
...
| **Queue Metrics** | 매칭 대기시간 분포, 큐 깊이 추이, 토큰 만료 건수 |
...
| Queue match_wait_ms 평균 > 30초 (5분 윈도우) | Warning | 매칭 지연 |
...
-- 매칭 대기시간 평균
SELECT avg(match_wait_ms) AS avg_wait
FROM project_sos.service_events
WHERE service = 'queue'
  AND match_wait_ms > 0
  AND timestamp >= now() - INTERVAL 5 MINUTE;
