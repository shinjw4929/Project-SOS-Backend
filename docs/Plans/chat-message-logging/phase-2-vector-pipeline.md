...
- [ ] **Task A**: `vector.toml`에 `route_cpp` 트랜스폼 추가 (카테고리 기반 분기)
- [ ] **Task B**: `vector.toml`에 `extract_chat_fields` 트랜스폼 추가 (key-value 파싱)
- [ ] **Task C**: `vector.toml`에 `clickhouse_chat_messages` 싱크 추가
- [ ] **Task D**: `clean_fields.inputs` 수정 (`parse_cpp` → `route_cpp._unmatched`)
- [ ] **Task E**: `vector-tests.toml`에 Chat:Message 파싱 테스트 추가
