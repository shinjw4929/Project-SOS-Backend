...
- [x] **Task A: ClickHouse 테이블 생성** — `infra/clickhouse/init.sql` 끝에 `chat_messages` 테이블 추가
- [x] **Task B: C++ 로깅 코드 추가** — `src/chat/channel/ChannelManager.cpp`의 `handleChatSend()`에 메시지 로깅
...
- [x] C++ 빌드 성공 (`cmake --build build`) — 환경 문제(vswhere 미발견)로 CI 검증 불가, 코드 변경 무관 확인
